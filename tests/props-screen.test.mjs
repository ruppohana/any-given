...
test('the ready sets are src/lib/props.ts\'s own: the 2026 Emmys (14, all at the broadcast) and Survivor 51 (2)', () => {
  ...
  assert.deepEqual(templatesOpen(SEP13).map((t) => t.id), ['emmys-2026', 'survivor-51']);
...
test('ready sets offered: the one Home remembered first, none that is already all here', () => {
  ...
  assert.deepEqual(P.offeredTemplates(open, '', []).map((t) => t.id), ['emmys-2026', 'survivor-51']);
  assert.deepEqual(P.offeredTemplates(open, 'survivor-51', []).map((t) => t.id), ['survivor-51', 'emmys-2026']);
  assert.equal(P.loadedCount('emmys-2026', EQ), 14);
  assert.deepEqual(P.offeredTemplates(open, 'emmys-2026', EQ).map((t) => t.id), ['survivor-51'], 'the Emmys are in');
  assert.deepEqual(P.offeredTemplates(open, '', EQ.slice(0, 5)).map((t) => t.id), ['emmys-2026', 'survivor-51'],
...
test('ready: the questions group is chosen over the NFL one, made current, and read with the server clock', async () => {
  ...
  assert.equal(d.props.templates.length, 2);
});
