...
# 🔴 NO CONSOLE WINDOW. Running node straight from the task opened a black box on
# the desktop — Jason photographed one and asked what it was. A background job
# that takes over the screen is not a background job.
#
# WScript.Shell.Run with window style 0 is the only route on Windows that shows
# nothing at all: `powershell -WindowStyle Hidden` still flashes its own host,
# and `start /min` leaves a taskbar button.
#
# 🔴 AND THE COMMAND IS BAKED INTO A PER-SPORT .vbs RATHER THAN PASSED TO ONE.
# schtasks caps /TR at 261 characters and the full line — wscript, the shared
# script, a 120-character node path, the repo path, the flags — came to more than
# that. It failed to register BOTH pollers, which is worse than the window it was
# fixing. A generated launcher makes /TR three short tokens.
$launcher = Join-Path $repo "tools\.poll-$Sport.vbs"
$cmdline = '"' + $node + '" "' + $script + '" ' + $GameId + ' --sport ' + $Sport + ' --every ' + $Every
@"
...
$tr = "wscript.exe //B //Nologo \`"$launcher\`""
